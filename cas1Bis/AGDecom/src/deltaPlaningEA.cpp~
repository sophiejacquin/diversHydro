...
typedef eoVector<double,eodeltaPlaning<MyFitT> > Indi;      // ***MUST*** derive from EO
...
try
  {
    ...
    eoState state;    // keeps all things allocated
     ...
     cout << "systeme créé dans deltaPlaningEA \n";
     // Decodeur decodeur(systeme);
   
    // The fitness
    //////////////
    eodeltaPlaningEvalFunc<Indi> plainEval/* (varType  _anyVariable) */;
      cout <<"fonction d'evaluation créée \n";
    // turn that object into an evaluation counter
    eoEvalFuncCounter<Indi> eval(plainEval);
     // cout << "là\n";
    // a genotype initializer
	
      eodeltaPlaningInit<Indi> init(systeme.getNbReservoirs(),systeme.getReservoirs(),30);
      cout <<"fonction d'initialisation créée \n";
    ...
    // A (first) crossover (possibly use the parser in its Ctor)
       double reserveAuto =systeme.getReservoirs()[1].getReserve();
      double ApportAnnuel= systeme.getReservoirs()[1].getApportAnnuel();
    
      eodeltaPlaningQuadCrossover cross(1,&(ApportAnnuel),&(reserveAuto)); /* 
        (eoParser parser) */
      cout <<"fonction croisement créée \n";
...
// cross.add(cross2, cross2Rate);
  ...
  eodeltaPlaningMutation<Indi> mut(systeme.getNbReservoirs(),systeme.getReservoirs()); /* (parser) */
 cout <<"fonction mutationcréée \n";
    // IF MORE THAN ONE:

    // read its relative rate in the combination
/*double mut1Rate = parser.createParam(0.5, "mut1Rate", "Relative rate for mutation 1", '1', "Variation Operators").value();
...
eoPropCombinedMonOp<Indi> mut(mut1, mut1Rate);
    ...
    // 3- uncomment and duplicate the following lines:
    //
 eodeltaPlaningSecondMutation<Indi> mut2(eoParser parser);
 ...
 // pop.sortedPrintOn(cout);
  ...
  //cout << "\n Meilleure Solution\n";
 //     char* titre= (char*)malloc(30*sizeof(char));;
  //    cout << "titre pour doc de sortie ?\n";
     // cin >> titre;
 // decodeur.affiche(pop.best_element(),"solution.txt");
 // cout << pop.best_element();
 // cout << endl;
 //free(titre);
  }
