...
private:
                     ...
                     if(resteADeverser<=resteADeverserJ &&(resteADeverser >=resteADeverserJ-kmax || contenu==Vmax  || i==nbSommets-1) )
                     {
                         ...
                         else
                         {
                             ...
                             if(quantite>=kmin )
                             {
                                 ...
                                 double eval=0;
                                 for(m=0;m<systeme.getNbTurbines();m++)
                                 {
                                     myobject ob;
                                     ob.nbTurbine =m;
                                     ob.coef=systeme.getTurbine(m).getW(inter);
                                 ...
                                 //Evaluation
                                 m=0;
                                 while(quantite>0 && m< systeme.getNbTurbines())
                                 {
                                     double coef = listeCoeff[m].coef;
                                     int numTurbine = listeCoeff[m].nbTurbine;
                                 ...
                                 sommets[i].setValeurArc(sommets[i].getNbPredecesseurs()-1,eval);
                             }
    ...
    double plusCourtChemin()
    {
        ...
        for(i=0;i<nbHeures;i++)
        {
            ...
            int pred = s.getPred();
            deverse= deverse + (s.getPredecesseurs(pred))->getContenuReservoir() + Apport[i] - reserve - s.getContenuReservoir();
       ...
       // cout << "qte deversée : " <<deverse<<endl;
        return eval ;
