    ...
    bool operator()(eodeltaPlaning<double> & _genotype)
    {
        ...
        else
        {
            ...
            if(pt1<pt2)
            {
                 ...
                 qn=reservoir.getVinit() - _genotype.getQuantiteSemaine(0) + reservoir.getApportPeriode(0,taille)-reservoir.getReserve()*8760/_genotype.getNbPeriodes();
                
                
                //Calcul de la borne sup:
                for(i=1;i<pt1+1;i++)
                {
                    qn=qn+reservoir.getApportPeriode(i,taille)-_genotype.getQuantiteSemaine(i)-reservoir.getReserve()*8760/_genotype.getNbPeriodes();
                }
