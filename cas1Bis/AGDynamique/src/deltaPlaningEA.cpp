 ...
 * class eodeltaPlaning MUST derive from EO<FitT> for some fitness
 */
#include "eodeltaPlaning.h"

/** definition of initilizqtion:
 * class eodeltaPlaningInit MUST derive from eoInit<eodeltaPlaning>
 */
#include "eodeltaPlaningInit.h"

/** definition of evaluation:
 * class eodeltaPlaningEvalFunc MUST derive from eoEvalFunc<eodeltaPlaning>
 * and should test for validity before doing any computation
 * see tutorial/Templates/evalFunc.tmpl
 */
#include "eodeltaPlaningEvalFunc.h"
 ...
 */
#include "eodeltaPlaningQuadCrossover.h"
#include "eodeltaPlaningSecondMutation.h"
#include "eodeltaPlaningMutation.h"

/* and (possibly) your personal statistics */
#include "eodeltaPlaningStat.h"

// GENOTYPE   eodeltaPlaning ***MUST*** be templatized over the fitness
...
typedef eodeltaPlaning<MyFitT> Indi;      // ***MUST*** derive from EO
...
try
  {
    ...
    eoState state;    // keeps all things allocated
       //Creation du systeme et des résultats dont on a besoin de maniere reccurente
      ...
      {
         sumQmax= sumQmax+ systeme.getTurbines()[i].getQMax();
      }
      //Vh qmax(h) :
      double* V;
      double* qmax;
      V= (double*)malloc(NBPERIODES* sizeof(double));
      qmax= (double*)malloc(NBPERIODES* sizeof(double));
      V[0]=systeme.getReservoirs()[1].getVinit()+ systeme.getReservoirs()[1].getApport()[0]-systeme.getReservoirs()[1].getReserve();
      qmax[0]=V[0]- Vmin;
      if(sumQmax<qmax[0]) qmax[0]= sumQmax;
      if(Qtot<qmax[0]) qmax[0]=Qtot;
      if(V[0]-Vmax> qmax[0])qmax[0]=V[0]-Vmax;
      for(i=1;i<NBPERIODES;i++)
      {
          V[i]=V[i-1]+ systeme.getReservoirs()[1].getApport()[i]-systeme.getReservoirs()[1].getReserve();
          qmax[i]=V[i]-Vmin;
          if(qmax[i-1]+sumQmax<qmax[i]) qmax[i]=qmax[i-1]+ sumQmax;
          if(Qtot<qmax[i]) qmax[i]=Qtot;
          if(V[i]-Vmax> qmax[i])qmax[i]=V[i]-Vmax;
      }
      Decodeur decodeur(systeme.getTurbines(),systeme.getNbTurbines());
     // cout <<"Qtot : "<<Qtot<<endl;
     // cout << V[8350]<<endl;
    //  cout<< V[342];
    // The fitness
    //////////////
      int nbPeriodes=NBPERIODES;
      eodeltaPlaningEvalFunc<Indi> plainEval(decodeur,V,systeme.getPrixSpot(),NBPERIODES,systeme.getReservoirs()[1].getVinit())/* (varType  _anyVariable) */;
    // turn that object into an evaluation counter
    eoEvalFuncCounter<Indi> eval(plainEval);
   
      
    // a genotype initializer
    eodeltaPlaningInit<Indi> init(V,Vmax,Vmin,Qtot,sumQmax,nbPeriodes);
    // or, if you need some parameters, you might as well
    // - write a constructor of the eodeltaPlaningInit that uses a parser
    // - call it from here:
    //        eodeltaPlaningInit<Indi> init(parser);
    ...
    eodeltaPlaningQuadCrossover<Indi> cross(NBPERIODES,V,Vmax,sumQmax,qmax); /* (eoParser parser) */
...
// eoPropCombinedQuadOp<Indi> cross(cross1, cross1Rate);
    ...
    // 1- write the new class by mimicking eodeltaPlaningQuadCrossover.h
    // 2- include that file here together with eodeltaPlaningQuadCrossover above
    // 3- uncomment and duplicate the following lines:
    //
// eodeltaPlaningSecondCrossover<Indi> cross2(eoParser parser);
...
// cross.add(cross2, cross2Rate);
  ...
  eodeltaPlaningMutation<Indi> mut1(PAS,qmax,sumQmax,Vmax,V,NBPERIODES); /* (parser) */
 ...
 eoPropCombinedMonOp<Indi> mut(mut1, mut1Rate);
	//cout <<"premiere mutation créée\n";
    // and as many as you want the following way:
    // 1- write the new class by mimicking eodeltaPlaningMutation.h
    // 2- include that file here together with eodeltaPlaningMutation above
    // 3- uncomment and duplicate the following lines:
    //
 eodeltaPlaningSecondMutation<Indi> mut2(Vmax,V,8760,24);/*(eoParser parser);*/
...
//cout<<"là!"<<endl;
  ...
  eodeltaPlaningStat<Indi>   myStat;       // or maybe myStat(parser);
  ...
  bool printdeltaPlaningStat = parser.createParam(false, "coutdeltaPlaningStat", "Prints my stat to screen, one line per generation", '\0', "My application").value();
  bool filedeltaPlaningStat = parser.createParam(false, "filedeltaPlaningStat", "Saves my stat to file (in resDir", '\0', "My application").value();
  bool plotdeltaPlaningStat = parser.createParam(false, "plotdeltaPlaningStat", "On-line plots my stat using gnuplot", '\0', "My application").value();

  // should we write it on StdOut ?
  if (printdeltaPlaningStat)
    {
